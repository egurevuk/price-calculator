...
<style>
    ...
    /* Labels */
    label, .stRadio label, .stNumberInput label, .stTextInput label {
...
# ── SECTION 2: Cost Type ──────────────────────────────────────────────────────
st.markdown("### Payout Cost Type")

cost_type = st.radio(
    "How is the payout cost charged?",
    options=["Fixed (USD per payout)", "Percentage (% of volume)"],
    horizontal=True,
    help="Choose whether the processing fee is fixed per transaction or a % of volume.",
    label_visibility="collapsed"
)

st.markdown("<br>", unsafe_allow_html=True)

# ── SECTION 3: Current vs Stape rates ────────────────────────────────────────
...
with curr_col:
    ...
    if cost_type == "Fixed (USD per payout)":
        current_payout_cost = st.number_input(
            ...
            min_value=0, value=3, step=1, format="%d",
...
with stape_col:
    ...
    if cost_type == "Fixed (USD per payout)":
        stape_payout_cost = st.number_input(
            ...
            min_value=0, value=1, step=1, format="%d",
...
if not has_errors:
    ...
    with st.expander("🔍 How this was calculated"):
        if cost_type == "Fixed (USD per payout)":
            st.markdown(f"""
**Formula:** `Cost = FX Rate × Volume + Payout Cost × Number of Payouts`
...
| Processing Cost | `${fmt_num(current_payout_cost)} × {fmt_num(amount_of_payouts)}` = **{fmt_usd(curr_proc_cost)}** | `${fmt_num(stape_payout_cost)} × {fmt_num(amount_of_payouts)}` = **{fmt_usd(stape_proc_cost)}** |
| **Total** | **{fmt_usd(curr_total)}** | **{fmt_usd(stape_total)}** |
| **Savings** | | **{fmt_usd(savings)}** ({savings_pct:.1f}%) |
            """)
        else:
            st.markdown(f"""
**Formula:** `Cost = FX Rate × Volume + Payout Cost (%) × Volume`

| | Current Provider | Stape |
|---|---|---|
| FX Cost | `{current_fx:.2f}% × {fmt_usd(payouts_volume)}` = **{fmt_usd(curr_fx_cost)}** | `{stape_fx:.2f}% × {fmt_usd(payouts_volume)}` = **{fmt_usd(stape_fx_cost)}** |
| Processing Cost | `{current_payout_cost:.2f}% × {fmt_usd(payouts_volume)}` = **{fmt_usd(curr_proc_cost)}** | `{stape_payout_cost:.2f}% × {fmt_usd(payouts_volume)}` = **{fmt_usd(stape_proc_cost)}** |
| **Total** | **{fmt_usd(curr_total)}** | **{fmt_usd(stape_total)}** |
| **Savings** | | **{fmt_usd(savings)}** ({savings_pct:.1f}%) |
            """)
