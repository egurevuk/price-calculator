...
STAPE_PAYOUT_FEE = 35    # USD per payout
...
<style>
    ...
    [data-testid="stMetricLabel"] { color: #c4b5e8 !important; font-size: 0.8rem !important; }
    ...
    .savings-card .label { color: #c4b5e8; font-size: 0.85rem; font-weight: 500; margin-bottom: 0.3rem; }
    .savings-card .value { color: #a78bfa; font-size: 2.4rem; font-weight: 800; line-height: 1.1; }
    .savings-card .sub   { color: #a78bfa; font-size: 0.9rem; margin-top: 0.4rem; opacity: 0.85; }
    ...
    /* Fixed pricing rows in Stape column */
    .fixed-fee-row {
        display: flex;
        align-items: center;
    ...
    .fixed-fee-row .fee-label {
        color: #c4b5e8;
        font-size: 0.82rem;
        font-weight: 500;
    }
    .fixed-fee-row .fee-value {
        color: #ffffff;
        font-size: 1.1rem;
        font-weight: 700;
    }
...
<div style="margin-bottom:0.5rem;">
    <img src="data:image/svg+xml;base64,{_LOGO_B64}" height="52" style="display:block;" />
</div>
""", unsafe_allow_html=True)
...
st.markdown("<p style='color:#c4b5e8; margin-bottom:1.5rem;'>Enter your current payout costs and see how much you save with Stape.</p>", unsafe_allow_html=True)

st.divider()

# ── SECTION 1: Payout Volume ──────────────────────────────────────────────────
...
with col2:
    raw_volume = st.text_input(
        "Payouts Volume (USD)",
        value="300 000",
        help="Total USD volume of all payouts."
    )
...
# ── Current Provider (fully editable) ────────────────────────────────────────
with curr_col:
...
# ── With Stape (fixed pricing, non-editable) ──────────────────────────────────
with stape_col:
    st.markdown('<div class="col-card"><h4>With Stape</h4>', unsafe_allow_html=True)

    st.markdown(f"""
    <div class="fixed-fee-row">
        <span class="fee-label">FX Rate</span>
        <span class="fee-value">{STAPE_FX_RATE:.1f}%</span>
    </div>
    <div class="fixed-fee-row">
        <span class="fee-label">Payout Fee (fixed per payout)</span>
        <span class="fee-value">${STAPE_PAYOUT_FEE}</span>
    </div>
    <span class="fixed-badge">🔒 Stape standard pricing</span>
    """, unsafe_allow_html=True)
...
if not has_errors:
    ...
    if savings > 0:
        ...
        <div class="savings-card" style="background:linear-gradient(135deg,rgba(200,50,50,0.2),rgba(150,30,30,0.15));border-color:rgba(255,100,100,0.3);">
            ...
            <div class="sub" style="color:#f87171;">Stape costs more in this scenario</div>
        </div>"""
    else:
        savings_html = """
        <div class="savings-card">
